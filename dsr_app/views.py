...
# pull blood type for a particular donor
def blood_out(bank, id, con):
    label = ''
    query = """SELECT "a+", "b+", "o+", "ab+", "a-", "b-", "o-", "ab-" FROM dsr_db5
            WHERE bankid='%s' AND donorid='%s'""" % (bank, id)
...
def donor_output():
    ...
    return render_template("output.html", donors=output, donors_sim=output_sim,
                           detection_message=message, the_result=[])
